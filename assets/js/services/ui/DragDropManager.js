...
export class DragDropManager {
    ...
    handlePointerMove(event) {
        ...
        if (!this.isDragging) {
            const dx = Math.abs(event.clientX - (this.startPosition.x + this.offset.x));
            const dy = Math.abs(event.clientY - (this.startPosition.y + this.offset.y));
        ...
        // Callback drag
        this.onDragCallback(this.draggedElement, this.dragData, { x: event.clientX, y: event.clientY });
    }
    ...
    handlePointerUp(event) {
        ...
        const isValid = this.onDragEndCallback(this.draggedElement, this.dragData, endPos);
