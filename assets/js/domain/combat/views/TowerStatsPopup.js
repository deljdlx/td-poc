...
export class TowerStatsPopup {
  ...
  show(tower) {
    ...
    this.debug.success("Tower stats shown (live updates enabled)", {
      towerId: tower.id,
    });
  }
  ...
  updateStats(deltaTime) {
    ...
    const accuracy =
      stats.shotsFired > 0
        ...
        : 0;
    const critRate =
      stats.hits > 0 ? ((stats.criticalHits / stats.hits) * 100).toFixed(1) : 0;
    const avgDamage =
      stats.hits > 0 ? (stats.totalDamage / stats.hits).toFixed(1) : 0;

    // Update only changed values
    if (this.statElements.shotsFired) {
      this.statElements.shotsFired.textContent =
        stats.shotsFired.toLocaleString();
    }
    ...
    if (this.statElements.accuracy) {
      ...
      this.statElements.accuracy.className =
        "stat-value " +
        (accuracy >= 70
          ? "stat-success"
          : accuracy >= 40
    ...
    if (this.statElements.totalDamage) {
      this.statElements.totalDamage.textContent =
