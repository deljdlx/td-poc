...
export class WaveManager {
  ...
  spawnEnemy(config) {
    ...
    const spawnPos = this.coordSystem.getElementCenter(
      startElement.cell.element,
    );

    // Create enemy with config
    const enemy = new Enemy(spawnPos.x, spawnPos.y);
    enemy.enemyType = config.type;
    enemy.attributes.health = config.health;
  ...
  onWaveComplete(waveNumber = 1) {
    ...
    const event = new WaveCompletedEvent(
      this.activeWave,
      waveNumber,
      totalSpawned,
    );
