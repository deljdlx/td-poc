...
export class GameDebugPanel {
  ...
  bindTimeScaleControls() {
    if (!this.gameClock) {
      console.warn(
        "GameDebugPanel: gameClock not set, time scale controls disabled",
      );
      return;
    }
  ...
  populateContextList() {
    ...
    contexts.forEach((context) => {
      ...
      checkbox.addEventListener("change", (e) =>
        this.onContextToggle(context, e.target.checked),
      );
  ...
  refreshTowersTab() {
    ...
    if (!this.game || !this.game.entityManager) {
      towersList.innerHTML =
        '<div style="color: #999; font-size: 11px; padding: 10px; text-align: center;">No game instance</div>';
      return;
    }
    ...
    if (towers.length === 0) {
      towersList.innerHTML =
        '<div style="color: #999; font-size: 11px; padding: 10px; text-align: center;">No towers placed</div>';
      return;
    }

    // Display tower info
    towersList.innerHTML = towers
      .map((tower, idx) => {
