...
export class GameStateService {
  ...
  start() {
    if (this.gameState.state !== GameState.READY) {
      this.debug.warning("Cannot start game - not in READY state", {
    ...
    const event = new GameStateChangedEvent(
      this.game,
      oldState,
  ...
  gameOver(reason = "Unknown") {
    ...
    // Emit state changed
    const stateEvent = new GameStateChangedEvent(
      this.game,
      oldState,
      this.gameState.state,
    );
    ...
    this.debug.error("💀 GAME OVER", {
      reason,
      finalScore: this.gameState.globalScore,
    });
  }
  ...
  nextWave() {
    ...
    const enemyCount =
      10 +
      (this.gameState.currentWaveNumber - 1) *
        this.gameState.config.waves.enemyIncrement;
